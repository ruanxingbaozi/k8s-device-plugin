...
func (m *NvidiaDevicePlugin) Allocate(ctx context.Context, reqs *pluginapi.AllocateRequest) (*pluginapi.AllocateResponse, error) {
	...
	for _, req := range reqs.ContainerRequests {
		...
		visibleDeviceMap := make(map[string]bool)
		visibleDevices := []string{}
		for _, v := range req.DevicesIDs {
			trueUUID := getTrueUUID(v)
			if _, found := visibleDeviceMap[trueUUID]; !found {
				visibleDeviceMap[trueUUID] = true
				visibleDevices = append(visibleDevices, trueUUID)
			}
		}
		...
		if *passDeviceSpecs {
			response.Devices = m.apiDeviceSpecs(visibleDevices)
		}
